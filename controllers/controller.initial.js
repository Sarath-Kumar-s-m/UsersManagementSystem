...
const login = async (req, res) => {
      ...
      if(req.method == 'GET' ){ //* if statement-1
       ...
       if(req.method == 'POST' ){  //^ if statement-2 
          ...
          try{
            ...
            if(findUser.length > 0 && findUser[0].role == 'user'){
               ...
               const findAdmin = await adminModel.find({email: email, password: password});
...
const signup = async (req, res) => {
      ...
      if(req.method == 'GET' ){ //! if statement-1
         ...
         if(req.session.role == 'user'){
          ...
          return res.status(200).render("signupForm",{ data:{
                     errorMsg: undefined,
   ...
   if(req.method == 'POST'){ //~ if statment-2
      ...
      try{
         ...
         if(!findUser[0]){
            ...
            if(!newUser){
               ...
               return res.status(404).render('signupForm',{ data:{
                          errorMsg: undefined,
                          emailErrorMsg: undefined,
